...
void llenar_vector(vector<int> & vector1){
    ...
    {
        if(elementos_leidos==vector1.size()){
            vector1.resize(vector1.size()*2+1);
        }
        vector1[elementos_leidos++]=dame_valor;
    }
    vector1.resize(elementos_leidos); //Para comprimir el vector solo a los espacios que usamos
